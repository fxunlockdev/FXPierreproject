...
describe('AlertDispatcher', () => {
  ...
  it('sends telegram alerts to the configured target', async () => {
    ...
    await dispatcher.dispatch('worker_offline', 'Relay offline', 'no heartbeat', 'space-1');
    expect(sendTelegram).toHaveBeenCalledWith('space-1', '@alerts', expect.stringContaining('Relay offline'));
  });

  it('respects per-trigger switches', async () => {
    const { dispatcher, sendTelegram } = make(settings({ triggers: { flood_wait: false } }));
    await dispatcher.dispatch('flood_wait', 'Rate limited', 'waiting 90s', 'space-1');
    expect(sendTelegram).not.toHaveBeenCalled();
  });

  it('applies the cooldown to repeats of the same alert', async () => {
    const { dispatcher, sendTelegram } = make(settings());
    await dispatcher.dispatch('worker_offline', 'Relay offline', 'first', 'space-1');
    await dispatcher.dispatch('worker_offline', 'Relay offline', 'second', 'space-1');
  ...
  it('keeps going when the telegram delivery fails', async () => {
    ...
    await expect(dispatcher.dispatch('worker_offline', 'Relay offline', 'x', 'space-1')).resolves.toBeUndefined();
  });
  ...
  it('never fetches a private webhook url', async () => {
    ...
    await dispatcher.dispatch('worker_offline', 'Relay offline', 'x', 'space-1');
